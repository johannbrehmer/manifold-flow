...
class RobotArmSimulator(BaseSimulator):
    def __init__(self, l1=1., l2=0.5, base_mean=0., base_std=1.0, phi1_mean=0.25*np.pi, phi1_std=0.125*np.pi, phi2_std=0.125*np.pi, noise=0.):
        self.l1 = l1
        self.l2 = l2
    ...
    def _observation(self, z):
        ...
        if self.noise > 0.:
            x += np.random.normal(0., self.noise, size=(n, 6))
