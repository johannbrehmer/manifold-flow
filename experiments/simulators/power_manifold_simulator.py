...
class PowerManifoldSimulator(BaseSimulator):
    def __init__(self, draw_constants=False, max_power=5, power_decay=3.0, base_coeff_std=1.0, exp_suppression=0.1, weight=0.4, const_width=2.0, min_width=0.2, max_width=1.0, filename=None):
    ...
    def _draw_z(self, n, parameters):
        categories = np.random.choice(2, size=n, replace=True, p=(1.-self._weight, self._weight))[:, np.newaxis]
    ...
    def _log_density_z(self, z, parameters):
        mean_fix = np.array([[1., -1.]])
        logp_fix = np.sum(norm.logpdf((z - mean_fix) / self._const_width), axis=-1)

        mean_var = np.array([[-1., 1.]])
        std_var = self._min_width + (self._max_width - self._min_width) * (0.5 + 0.5*parameters.reshape((-1, 1)))
        logp_var = np.sum(norm.logpdf((z - mean_var) / std_var), axis=-1)
