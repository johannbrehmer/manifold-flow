...
def pick_parameters(args, trial, counter):
    ...
    margs.outerlayers = trial.suggest_categorical("outerlayers", [2, 3, 5, 8])
    margs.innerlayers = trial.suggest_categorical("innerlayers", [2, 3, 5, 8])
    ...
    margs.outercouplinghidden = trial.suggest_categorical("outercouplinghidden", [10, 20, 50, 100])
    margs.dropout = trial.suggest_categorical("dropout", [0.0, 0.20])
    margs.splinerange = trial.suggest_uniform("splinerange", 4.0, 8.0)
    ...
    margs.weightdecay = trial.suggest_loguniform("weightdecay", 1.0e-9, 1.0e-3)
