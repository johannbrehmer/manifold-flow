...
def simulator(epsilon, latent_dim, data_dim, n, transform):
    z = np.random.normal(0.0, 1.0, size=(n, latent_dim))
    logp = np.log(norm(loc=0.0, scale=1.0).pdf(z))
...
def true_logp(x, epsilon, latent_dim, data_dim, transform, only_submanifold=False):
    ...
    logp = np.log(norm(loc=0.0, scale=1.0).pdf(z[:, :latent_dim]))
    if latent_dim < data_dim and not only_submanifold:
        prob_eps = norm(loc=0.0, scale=epsilon).pdf(z[:, latent_dim:])
        logp_eps = np.log(np.clip(prob_eps, 1.0e-3, 1.0e3))
        logp = np.concatenate((logp, logp_eps), axis=1)
...
def generate(
    ...
    transform = np.random.normal(
        loc=0.0, scale=1.0, size=(max(data_dims), max(data_dims))
    )
    np.save("{}/data/gaussian/gaussian_transform.npy".format(base_dir), transform)

    for data_dim in data_dims:
        x_train, z_train, logp_train = simulator(
            epsilon, latent_dim, data_dim, n_train, transform
        )
        x_test, z_test, logp_test = simulator(
            epsilon, latent_dim, data_dim, n_test, transform
        )
        ...
        np.save(
            "{}/data/gaussian/gaussian_{}_{}_x_test.npy".format(
                base_dir, latent_dim, data_dim
            ),
            x_test,
        )
        np.save(
            "{}/data/gaussian/gaussian_{}_{}_z_train.npy".format(
                base_dir, latent_dim, data_dim
            ),
            z_train,
        )
        np.save(
            "{}/data/gaussian/gaussian_{}_{}_z_test.npy".format(
                base_dir, latent_dim, data_dim
            ),
            z_test,
        )
        np.save(
            "{}/data/gaussian/gaussian_{}_{}_logp_train.npy".format(
                base_dir, latent_dim, data_dim
            ),
            logp_train,
        )
        np.save(
            "{}/data/gaussian/gaussian_{}_{}_logp_test.npy".format(
                base_dir, latent_dim, data_dim
            ),
            logp_test,
        )
