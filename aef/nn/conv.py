...
class GatedConv2d(nn.Module):
    def __init__(
        ...
        self.conv = nn.Conv2d(
            in_channels, 2 * out_channels, kernel_size, stride, padding, dilation
        )
...
class SylvesterFlowConvEncoderNet(nn.Module):
    def __init__(self, context_features, last_kernel_shape=(7, 7)):
        ...
        self.gated_conv_layers = nn.ModuleList(
            [
                GatedConv2d(
                    in_channels=1, out_channels=32, kernel_size=5, padding=2, stride=1
                ),
                GatedConv2d(  # 2
                    in_channels=32, out_channels=32, kernel_size=5, padding=2, stride=2
                ),
                GatedConv2d(  # 3
                    in_channels=32, out_channels=64, kernel_size=5, padding=2, stride=1
                ),
                GatedConv2d(  # 4
                    in_channels=64, out_channels=64, kernel_size=5, padding=2, stride=2
                ),
                GatedConv2d(  # 5
                    in_channels=64, out_channels=64, kernel_size=5, padding=2, stride=1
                ),
                GatedConv2d(  # 6
                    in_channels=64, out_channels=64, kernel_size=5, padding=2, stride=1
                ),
        ...
        self.context_layer = nn.Linear(
            in_features=256, out_features=self.context_features
        )
    ...
    def __init__(self, latent_features, last_kernel_shape=(7, 7)):
        ...
        self.gated_conv_transpose_layers = nn.ModuleList(
            [
                ...
                GatedConv2d(  # 7
                    in_channels=32, out_channels=1, kernel_size=1, padding=0, stride=1
                ),
            ]
        )
...
class ResidualBlock(nn.Module):
    def __init__(
        self, in_channels, resample=None, activation=F.relu, dropout=None, first=False
    ):
        super().__init__()
    ...
    def __init__(self, latent_features, channels_multiplier, activation=F.relu):
        ...
        self.residual_blocks = nn.ModuleList(
            [
                ...
                ResidualBlock(
                    in_channels=channels_multiplier * 8, resample="up", first=True
                ),
                ResidualBlock(in_channels=channels_multiplier * 4),
